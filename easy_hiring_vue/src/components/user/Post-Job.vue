<template>
  <div>
    <v-row>
      <v-col md="12" style="margin-top: -100px;">
        <div>
          <v-banner class="ma-3">
            <p class="text-h5 indigo--text mb-0">Write your Inside Job</p>
            <span class="caption">Please fill up the below form to post a Job</span>
          </v-banner>
          <v-form class="pa-5 ma-10" ref="form" v-model="valid" lazy-validation @submit.prevent="submitJob">
            ...
            <v-select :rules="nameRules" v-model="salaryId" :items="salaries" item-text="name" item-value="id"
              label="Select a Salary"></v-select>

            <v-div>

              <v-select :rules="nameRules" v-model="locationId" :items="locations" item-text="name" item-value="id"
                label="Select a location"></v-select>
            </v-div>
            <v-text-field :rules="nameRules" v-model="level" label="level" required></v-text-field>
            <v-row>
              ...
              <v-col cols="12" sm="6" md="4">
                ...
                <v-div>
                  ...
                  <v-select :rules="nameRules" v-model="categoryId" :items="items" item-text="name" item-value="id"
              ...
              <v-col>
                <v-div>
                  ...
                  <v-file-input :rules="nameRules" v-model="image" label="Upload Cover Image" chips
...
export default {
  ...
  async created() {
    try {
      ...
      const response3 = await axios.get(`${API_LOCATION}/salary`);
      console.log(response3)
      this.salaries = response3.data.salary
  ...
  methods: {
    ...
    async submitJob() {
      ...
      formData.append('salaryId', this.salaryId)
